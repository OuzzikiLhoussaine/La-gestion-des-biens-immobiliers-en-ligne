...
// function for the Admin email To check Exists OR NOT.
	...
	// Checks for Agent Email Exist or Not
    function agent_email_exists($email,$con)
	{
		$row=mysqli_query($con, "SELECT `email` FROM `agents` WHERE `email`='$email' ");
		{
			if(mysqli_num_rows($row)==1) 
			{
				return true;
	...
	// Checks for Agent CNIC Already Exist or Not
	function agent_cnic_exists($cnic_no,$con)
	{
		$row=mysqli_query($con, "SELECT `cnic_no` FROM `agents` WHERE `cnic_no`='$cnic_no' ");
		{
			if(mysqli_num_rows($row)==1) 
			{
				return true;
